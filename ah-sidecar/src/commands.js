...
async function cmdRegister({ user }) {
...
async function cmdBalance({ user }) {
  await ensurePlayer(user)
  return { message: `Your balance: ${formatGold(await getBalance(user))}` }
}

async function cmdList({ user, search, page }) {
  await ensurePlayer(user)
  const listings = await getActiveListings({ search, page: page ?? 0 })
  if (!listings.length) {
...
async function cmdDetail({ user, listingId }) {
  await ensurePlayer(user)
  const l = await getListing(listingId)
  if (!l || l.status !== 'active') return { message: 'Listing not found.' }
  const history = (await getBidHistory(listingId)).slice(0, 5)
...
async function cmdSell({ user, itemName, itemFormId, quantity, minBid, buyoutPrice, durationHours }) {
  await ensurePlayer(user)
...
async function cmdBid({ user, listingId, amount }) {
  await ensurePlayer(user)
  const result = await placeBid(user, listingId, amount)
  if (!result.ok) return { message: `[AH] ${result.error}` }
  return {
...
async function cmdBuyout({ user, listingId }) {
  await ensurePlayer(user)
  const result = await executeBuyout(user, listingId)
  if (!result.ok) return { message: `[AH] ${result.error}` }
  return {
    message: `[AH] You bought "${result.itemName}" x${result.quantity} for ${formatGold(result.price)}. Collect your item from the Auction Chest.`,
    broadcast: `[AH] ${user} bought "${result.itemName}" from ${result.seller} for ${formatGold(result.price)}!`
  }
}

async function cmdCancel({ user, listingId }) {
  await ensurePlayer(user)
  const result = await cancelListing(user, listingId)
  if (!result.ok) return { message: `[AH] ${result.error}` }
  return { message: `[AH] Cancelled listing "${result.itemName}". Note: deposit fee was forfeited. Retrieve your item from the Auction Chest.` }
}

async function cmdMyListings({ user }) {
  await ensurePlayer(user)
  const listings = await getMyListings(user)
...
async function cmdMyBids({ user }) {
  await ensurePlayer(user)
  const bids = await getMyBids(user)
...
async function cmdMailbox({ user }) {
  await ensurePlayer(user)
  const deliveries = await getPendingDeliveries(user)
...
async function cmdClaim({ user, deliveryId }) {
  await ensurePlayer(user)
  const result = await claimDelivery(deliveryId, user)
